...
@section('content')
                            ...
                            <div class="table-responsive tbl-responsive">
                                <table class="table text-nowrap" cellpadding="10">
                                    <tbody>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <div>
                                                        ...
                                                        <?php if ($details->deposit_proof != "" && $details->deposit_proof != NULL) { ?>
                                                        <div class="mt-3">
                                                            <a href="{{ asset('storage/' . $details->deposit_proof) }}"
                                                                target="_blank">
                                                                <img style="height: 50px;"
                                                                    src="{{ asset('storage/' . $details->deposit_proof) }}"
                                                                    alt="Deposit Proof">
                                                            </a>
                                                        </div>
                                                        <?php } ?>
                                                        <div class="d-flex align-items-center">
                                                            <div>
                                                              <?php if ($details->deposit_currency != 'USD' && !empty($details->deposit_currency)) { ?>
                                                                <div class="lh-1 mt-2">
                                                                  <span class="fs-11 text-muted">DEPOSIT AMOUNT IN <?= $details->deposit_currency ?></span>
                                                                </div>
                                                                <div class="lh-1 mt-2">
                                                                  <span><?= $details->deposit_currency ?>       <?= $details->deposit_currency_amount ?></span>
                                                                </div>
                                                                <div class="lh-1 mt-2">
                                                                  <span class="fs-11 text-muted">ADJUSTMENT AMOUNT</span>
                                                                </div>
                                                                <div class="lh-1 mt-2">
                                                                  <span><?= $details->deposit_currency ?>       <?= $details->adj_amount ?></span>
                                                                </div>
                                                              <?php } ?>
                                                            </div>
                                                          </div>
                                        ...
                                        <tr>
                                            @if ($details->deposit_type == 'BankWire' && !empty($details->bankwire_details))
                                                <td>
                                                    <div class="lh-1">
                                                        <span class="fs-11 text-muted">BANKWIRE DETAILS</span>
                                                    </div>
                                                    <div class="mt-1">
                                                        {!! $details->bankwire_details !!}
                                                    </div>
                                                </td>
                                                <td colspan="1">
                                                </td>
                                            ...
                                            <td>
                                                <div class="btn-list ms-auto my-auto">
                                                    <button
                                                        onclick="takeAction('{{ $details->email }}','{{ $details->deposit_amount }}',1,{{ $details->trade_id }},'{{ $details->bonus_amount }}')"
                                                        type="button"
                                                        class="btn btn-success btn-space m-1">Approve</button>
                                                    <button
                                                        onclick="takeAction('{{ $details->email }}','{{ $details->deposit_amount }}',2,{{ $details->trade_id }},'{{ $details->bonus_amount }}')"
                                                        type="submit"
    ...
    <script>
        function takeAction(email, amount, status, tradeId, bonus_amount) {
            Swal.fire({
                title: `Are you sure you want to ${status === 1 ? "approve" : "reject"} this transaction?`,
                html: `
            <form id="updateTransactionForm" method="post" action="{{ route('admin.updateTransaction') }}">
              @csrf
