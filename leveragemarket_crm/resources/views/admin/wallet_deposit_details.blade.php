                                                ...
                                                <div class="d-flex align-items-center">
                                                    <div>
                                                        ...
                                                        <?php if ($details->deposit_proof != "" && $details->deposit_proof != NULL) { ?>
                                                            <div class="mt-3">
                                                                <?php
                                                                $file_extension = pathinfo($details->deposit_proof, PATHINFO_EXTENSION);
                                                                if (in_array(strtolower($file_extension), ['jpg', 'jpeg', 'png'])) { ?>
                                                                    <a href="{{ asset('storage/' . $details->deposit_proof) }}" target="_blank">
                                                                        <img style="height: 50px;" src="{{ asset('storage/' . $details->deposit_proof) }}" alt="Deposit Proof">
                                                                    </a>
                                                                <?php
                                                                } elseif (strtolower($file_extension) == 'pdf') { ?>
                                                                    <a href="{{ asset('storage/' . $details->deposit_proof) }}" target="_blank">
                                                                        <span class="text-primary text-decoration-underline">View Deposit Proof</span>
                                                                    </a>
                                                                <?php } ?>
                                                            </div>
                                                        <?php } ?>
                                        ...
                                        <tr>
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <div>
                                                        <div class="lh-1">
                                                            <span class="fs-11 text-muted">COMPANY BANK</span>
                                                        </div>
                                                        <div class="lh-1 mt-2">
                                                            <span>{{ $details->company_bank }}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td>
                                                <div class="btn-list ms-auto my-auto">
                                                    <button
                                                        onclick="takeAction('{{ $details->email }}','{{ $details->deposit_amount }}',1)"
                                                        type="button"
                                                        class="btn btn-success btn-space m-1">Approve</button>
                                                    <button
                                                        onclick="takeAction('{{ $details->email }}','{{ $details->deposit_amount }}',2)"
                                                        type="submit"
    ...
    <script>
        function takeAction(email, amount, status) {
          ...
          <form id="updateTransactionForm" method="post">
            ...
            <textarea name="description" rows="3" class="mt-2 form-control" placeholder="Add a description"></textarea>
          </form>
        `,
                focusConfirm: false,
                ...
                preConfirm: () => {
                    ...
                    if (!description) {
                        Swal.showValidationMessage('Please add a description');
                        return false;
                    }
                    return true;
                }
