    ...
    <script>
        ...
        function dTSelection() {
            ...
            $('.ajaxDataTable tbody tr').on('click', '.ibToggle', function() {
                ...
                // console.log(data);
                $("#ibRequestForm input:not([name='_token']),#ibRequestForm select").val("").trigger("change");
            ...
            window.dTtable = $('#tableIbUsers').on("draw.dt", dTSelection).DataTable({
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, row, row_data) {
                            if (data == 1) {
                                return "<button class='ibToggle badge btn-sm btn btn-outline-success'>Active IB</button>   <a href='/admin/ibclientdetails?ibid=" + row_data.enc +
                                "' target='_blank' ><button class='badge btn-sm btn btn-outline-warning'>View</button></a>";
                            } else if (data == 2) {
                                return "<button class='ibToggle badge btn-sm btn btn-outline-danger'>Rejected</button>   <a href='/admin/ibclientdetails?ibid=" + row_data.enc +
                                "' target='_blank' ><button class='badge btn-sm btn btn-outline-warning'>View</button></a>";
                            } else if (data == 0) {
                                return "<button class='ibToggle badge btn-sm btn btn-outline-info'>IB Requested</button>   <a href='/admin/ibclientdetails?ibid=" + row_data.enc +
                                "' target='_blank' ><button class='badge btn-sm btn btn-outline-warning'>View</button></a>";
                            } else {
                                return "<button class='ibToggle badge btn-sm btn btn-outline-primary'>Not Requested</button> <a href='/admin/ibclientdetails?ibid=" + row_data.enc +
                                "' target='_blank' ><button class='badge btn-sm btn btn-outline-warning'>View</button></a>";
                            }
                        }
                    },
                    ...
                    /*{ 	data: 'action', 
						...
						searchable: false, 
						render: function(data, row, row_data) {
							return '<a href="/admin/?ibid=' + row_data.enc + '"><button class="badge btn-sm btn btn-outline-info">View</button></a>';
						}
					}*/
                ]
            });
