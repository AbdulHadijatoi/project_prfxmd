...
class MTLogger
   ...
   */
  // public static function Init($agent, $is_write_log = true, $file_path = '/var/www/libertygroup_crm/storage/logs/mt5', $file_prefix = '')
  //   {
    //--- set first data
    ...
    // // Default to Laravel's storage/logs/mt5
    // if (empty($file_path)) {
    //     $file_path = base_path('storage/logs/mt5');
    // }
    ...
    public static function Init($agent, $is_write_log = true, $file_path = null, $file_prefix = '')
{
    MTLogger::$m_agent = $agent;
    MTLogger::$m_is_write_log = $is_write_log;
    MTLogger::$m_file_prefix = $file_prefix;
    ...
    if (empty($file_path)) {
        $file_path = storage_path('logs/mt5'); // Laravel safe default
    }
...
{
    MTLogger::$m_file_path = rtrim($file_path, '/\\') . '/';

    // Only try to create if the directory truly doesn't exist
    if (!is_dir(MTLogger::$m_file_path)) {
        if (!@mkdir(MTLogger::$m_file_path, 0777, true) && !is_dir(MTLogger::$m_file_path)) {
            throw new \RuntimeException('Cannot create log directory: ' . MTLogger::$m_file_path);
        }
    }
}
  // public static function setFilePath($file_path)
