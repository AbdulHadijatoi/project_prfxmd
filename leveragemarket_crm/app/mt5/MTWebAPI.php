...
//+------------------------------------------------------------------+
//--- web api version
define("WebAPIVersion", 4410);
//--- web api date
define("WebAPIDate", "21 Jun 2024");
//--- including all classes


include "MTAuthProtocol.php";
include "MTUtils.php";
include "MTConnect.php";
include "MTProtocolConsts.php";
include "MTLogger.php";
include "MTTimeProtocol.php";
include "MTCommonProtocol.php";
include "MTGroupProtocol.php";
include "MTSymbolProtocol.php";
include "MTUserProtocol.php";
include "MTDealProtocol.php";
include "MTOrderProtocol.php";
include "MTPositionProtocol.php";
include "MTHistoryProtocol.php";
include "MTTickProtocol.php";
include "MTMailProtocol.php";
include "MTNewsProtocol.php";
include "MTPingProtocol.php";
include "MTTradeProtocol.php";
include "MTCustomProtocol.php";
include "MTServer.php";
...
class MTWebAPI
  ...
  // public function __construct($agent = 'WebAPI', $file_path = '/var/www/libertygroupfx_crm/storage/logs/MT5_logs/', $is_crypt = true)
  //   {
  //   $this->m_agent    = $agent;
  //   $this->m_is_crypt = $is_crypt;
  //   MTLogger::Init($agent, true, $file_path);
  //   }
public function __construct($agent = 'WebAPI', $file_path = null, $is_crypt = true)
{
    $this->m_agent    = $agent;
    $this->m_is_crypt = $is_crypt;

    // Use default Laravel storage path if $file_path not provided
    if (empty($file_path)) {
        $file_path = storage_path('logs/MT5_logs');
    }

    // Make sure folder exists
    if (!is_dir($file_path)) {
        mkdir($file_path, 0775, true);
        chown($file_path, 'www-data');  // or apache/nginx user
        chgrp($file_path, 'www-data');  // same
    }
