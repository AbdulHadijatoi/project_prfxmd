...
function addIpLog($action, $data, $email = null)
{
    
    $user = null;
    if(Auth::check()){
        $user = DB::table('emplist')->where('client_index', Auth::user()->id)->first();
    }
    $ip = request()->ip();
    $email = $user && $user->email ? $user->email : ($email ?? session('clogin'));
    DB::table('login_history')->insert([
        'email' => $email,
        'ip' => $ip,
        'country' => null,
        'action' => $action,
        'datalog' => json_encode($data, true),
